...
public class Trains {
  ...
  public List<Trains> getDateTicket()	{ return getDateTicket; }
  public int getID()            { return newDateTicketID; }
  ...
  public void getDateTicketOrderList()
  {
	  ...
	  //newDateTicketID = 0;
      for (Trains o: getDateTicket()) { //newOrderID should be max of all existing ones
  ...
  public Trains(int nID, String nDepart, String nDestination, String nDate, String nTime, double nPrice) {
    id = nID;      depart = nDepart;    destination = nDestination;		date = nDate;	time = nTime;	price = nPrice;
  }
  
  public Trains(int bdID, int bdtID, String bdDate){
	  newDateTicketID = bdID;	BDTID = bdtID;	BDDATE = bdDate;
  }

  public String toString() {
	  //return format string will be nice
	  return String.format("%4d	%20s	%20s	%10s	%10s	%10.2f", id, depart, destination, date, time, price); 
    //return new String(id+"\t\t"+depart+"\t\t"+destination+"\t\t"+date+"\t\t"+time+"\t\t"+price);   
  }
  ...
  public String showTicket() {
  	//String s = new String ("id\t\tdepart\t\tdestination\t\tdate\t\ttime\t\tprice\n");
